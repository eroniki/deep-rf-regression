...
def by_xy(x_train, y_train, x_test, y_test, log_dir):
    callback = [
        TensorBoard(log_dir=log_dir, histogram_freq=10,
...
def fit_gaussian(X):
    gmm = mixture.GaussianMixture(
        n_components=1, covariance_type='full').fit(X)
    gmm.predict(X)
...
def showTSNE(data, labels, nclass):
    ...
    cax = ax.scatter(t[:, 0], t[:, 1], labels(
    ), color=colors[dataset.grid_numbers_train[0:300].ravel(), :])
    plt.show()
...
def main():
    ...
    # Create the dataset
    dataset = wifi_data.wifi_data(folder_location=folderLocation, filename=fname_dataset,
                                  normalize=False, missingValues=0, nTraining=1200, nTesting=343, nValidation=0, verbose=False)
    ...
    grid_numbers_train_categorical = to_categorical(
        dataset.grid_numbers_train, 175)
    grid_numbers_test_categorical = to_categorical(
        dataset.grid_numbers_test, 175)
    ...
    # print np.amin(dataset.wifi_train), np.amax(dataset.wifi_train),
    # np.count_nonzero(np.isnan(dataset.wifi_train))
    accxy, _ = naiveBayes(dataset.lora_train, dataset.grid_numbers_train.ravel(
    ), dataset.lora_test, dataset.grid_numbers_test.ravel())
    accx, probx = naiveBayes(dataset.lora_train, dataset.grid_labels_train[:, 0].ravel(
    ), dataset.lora_test, dataset.grid_labels_test[:, 0].ravel())
    accy, proby = naiveBayes(dataset.lora_train, dataset.grid_labels_train[:, 1].ravel(
    ), dataset.lora_test, dataset.grid_labels_test[:, 1].ravel())
    print probx.shape, proby.shape
    # print np.min(dataset.grid_labels_test[:,0]), np.max(dataset.grid_labels_test[:,0])
    # print np.min(dataset.grid_labels_test[:,1]),
    # np.max(dataset.grid_labels_test[:,1])

    # print np.array([np.argmax(probx, axis=1), np.argmax(proby, axis=1)]) ==
    # dataset.grid_labels_test
    sx = 0
    sy = 0
    plt.ion()
    fig = plt.figure()
    ...
    for i in range(0, 343):
        ...
        im = ax.imshow(belief, extent=[
                       0, 6, 24, 0], aspect='auto', interpolation='gaussian', origin='upper')
    ...
    e = localization_error(dataset.grid_labels_test, np.vstack(
        (np.argmax(probx, axis=1), np.argmax(proby, axis=1))).transpose())
    ...
    # print by_number(dataset.lora_train, grid_numbers_train_categorical,
    # dataset.lora_test, grid_numbers_test_categorical, "./logs")
